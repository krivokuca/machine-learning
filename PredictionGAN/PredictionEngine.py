"""
TimeSeries Generative Adversarial Network (TSGAN)
Version: 1.0.0

TODO:
    * Add multiple ticker support for a model
    * Squash each generator/discriminator model pair into one savefile (instead of two)
    * Add actual error handling instead of returning False on error
    * Add CUDA toggle (currently cuda is used at all times)
...
class PredictionEngine:
    ...
    def train(self, full=False, period=False, tickers=False, output_path=False, window_size=False, epochs=400, crit_interval=4, learning_rate=False):
        ...
        if not full:
            ...
            checkpoints = {}
            # try finding a file in the model storage directory that matches the given window size and stock parameters
        ...
        # get and format the stock data and spectrogram data
        input_tensors = []
        input_specgrams = []
